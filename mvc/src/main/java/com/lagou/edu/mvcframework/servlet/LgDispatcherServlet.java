        ...
        for(Map.Entry<String,Object> entry: ioc.entrySet()) {
            ...
            for (int i = 0; i < methods.length; i++) {
                ...
                if(method.isAnnotationPresent(Security.class)){
                    ...
                    methodSecurity= security.values();
                }

                //如果controller上加了@Security注解，需要对所有的方法赋予对应用户的权限
                if(values !=null && values.length > 0){
                    Set<String> securitySet=new HashSet<>();
                    securitySet.addAll(Arrays.asList(values));
                    //如果方法上也加了该注解，需要对权限进行合并
                    if(methodSecurity !=null && methodSecurity.length > 0){
                        securitySet.addAll(Arrays.asList(methodSecurity));
                    }
                    handler.getSecurityMap().put(method,securitySet);
                }
...
//        method.invoke() //
        ...
        Map<Method, Set<String>> securityMap = handler.getSecurityMap();
        Method method = handler.getMethod();
        if(securityMap!=null && securityMap.containsKey(method) && !securityMap.get(method).contains(userName)){
            resp.getOutputStream().write("用户无操作权限".getBytes("GB2312"));
        }
        if(handler == null) {
        ...
        try {
            method.invoke(handler.getController(),paraValues);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
